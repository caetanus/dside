...
pass=0 fail=0
mapfile -t targets < <(./build --list 2>/dev/null | sed -n 's/^- //p')
if [ "${#targets[@]}" -eq 0 ]; then echo "# ERROR: ./build --list produced no targets" >&2; exit 2; fi
for t in "${targets[@]}"; do
  case "$t" in $filter) ;; *) continue ;; esac
  log="$logdir/$t.log"
  start=$(date +%s%3N)
  if ./build "$t" >"$log" 2>&1; then st=pass; pass=$((pass+1)); rm -f "$log"; logcol=-
  else st=fail; fail=$((fail+1)); logcol="$log"; fi
  ms=$(( $(date +%s%3N) - start ))
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%d\t%s\n' \
    "$t" "$(category "$t")" "$(compiler "$t")" "$(qtaxis "$t")" "$(optional "$t")" "$st" "$ms" "$logcol"
done
printf '# totals: %d pass, %d fail (gated targets absent from ./build --list are not shown)\n' "$pass" "$fail"
