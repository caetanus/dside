...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // A document we hand to the ENGINE whole, rather than compile. The generated class is a
        // holder — the same containment an engine-built CHILD already uses, applied at the root —
        // so the object IS the engine's and every question about it goes through its interface.
    ...
    // `--objpaths` hands the oracle, resolved by name at runtime, because a holder has no fields to
    ...
    if (delegateDoc) {
        ...
        for (auto &l : dl) {
            ...
            std::string path = l.label.substr(0, lp);
            if (path.find('@') != std::string::npos) continue;
            if (!seenObj.insert(l.setObj).second) continue;
            paths.push_back(path);
        }
        std::fprintf(stderr, "qmltc-d: %s: DOCUMENT DELEGATED to the engine — not compiled "
                     "(%zu object paths dumped through it)\n", inPath, paths.size() + 1);
        std::printf("// GENERATED by qmltc-d from %s — DELEGATED, not compiled.\nmodule %s;\n", inPath, qPrintable(cls));
        ...
        std::printf("\n/// %s, handed to the engine whole. The object is the ENGINE'S; this class only\n"
        ...
        if (dump && !noMain) {
            ...
            std::printf("    foreach (a; args[1 .. $]) {\n"
                        ...
                        "    foreach (a; args) if (a.length > 6 && a[0 .. 6] == \"--set:\") {\n"
                        "        auto eq = a.indexOf('='); if (eq < 0) continue;\n"
                        "        setProp(o.__inst, a[6 .. eq], a[eq + 1 .. $]);\n    }\n");
            std::printf("    foreach (a; args) if (a == \"--dumpall\") {\n"
                        "        qtd_dump_object_as(o.__inst, \"\", \"%s\");\n", rootType.c_str());
            for (auto &pp : paths)
                std::printf("        dumpPath(o.__inst, \"%s\");\n", pp.c_str());
            std::printf("    }\n}\n");
        }
        return 0;
    }
