...
// THE OUTPUT NOTICE — the same one xiboca writes, emitted by the OTHER generator in this
// repository (round 15 #5).
//
// Phase 3 of the licensing plan says every emitted `.d` and `.cpp` carries the SPDX header, the
// grant over generated output and a provenance line. It had been implemented in xiboca only,
