...
// THE OUTPUT NOTICE — the same one generator-d writes, emitted by the OTHER generator in this
// repository (round 15 #5).
//
// Phase 3 of the licensing plan says every emitted `.d` and `.cpp` carries the SPDX header, the
// grant over generated output and a provenance line. It had been implemented in generator-d only,
// and qmltc-d — whose output is precisely what a user compiles into their own program, outside any
// package this project builds — opened with one bare `// GENERATED by qmltc-d …` line. The file
// that most needs to answer "what may I do with this, and what made it" was the one that could not.
//
// It goes on all four outputs: the compiled document, the delegated document, the one-refused-
// expression variant, and the `--shadow-dir` QML. `notice=` is the version of this text; when the
// grant wording changes, that number changes with it, so a file records which policy it was emitted
// under rather than only when.
//
// `inputSha` is the round 15 #6 half: a revision alone does not identify the input, and two
// different documents with the same basename in two checkouts produced identical provenance.
// Supplied by the BUILD, in its own translation unit (see qmltcTool in reggaefile.d). This used to
// shell out to `git rev-parse` in the current directory at emission time — which answers "what is
// the checkout I am being RUN from", not "what produced this compiler". Measured on the same binary
// with the same input: `a0b3b94-dirty` from this tree, `unknown` from /tmp, and inside another
// repository it would have recorded that repository's revision as the generator's.
extern "C" const char *qtd_gen_rev(void) __attribute__((weak));

static std::string qtdGenRev()
{
    if (qtd_gen_rev) {
        const char *r = qtd_gen_rev();
        if (r && *r) return r;
    }
    return "unknown";
}

static std::string qtdFileSha(const char *path)
{
    QFile f(QString::fromUtf8(path));
    if (!f.open(QIODevice::ReadOnly)) return "unknown";
    QCryptographicHash h(QCryptographicHash::Sha256);
    if (!h.addData(&f)) return "unknown";
    return h.result().toHex().left(12).toStdString();
}

// `what` names the kind of output, because a delegated document and a compiled one are different
// artifacts and the reader should not have to infer which they hold.
static void qtdEmitNotice(FILE *out, const char *inPath, const char *what)
{
    std::fprintf(out,
        "// GENERATED by qmltc-d from %s — %s. Do NOT edit; regenerate.\n"
        "//\n"
        "// SPDX-FileCopyrightText: 2026 Marcelo A Caetano\n"
        "// SPDX-License-Identifier: BSL-1.0\n"
        "//\n"
        ...
        "//\n"
        "// provenance: generator=%s qt=%s tool=qmltc-d input=%s inputsha256=%s notice=v1\n",
        inPath, what, qtdGenRev().c_str(), QT_VERSION_STR,
        QFileInfo(QString::fromUtf8(inPath)).fileName().toUtf8().constData(),
        qtdFileSha(inPath).c_str());
}
...
int main(int argc, char **argv) {
    ...
    if (delegateDoc) {
        ...
        qtdEmitNotice(stdout, inPath, "DELEGATED to the engine, not compiled");
        std::printf("module %s;\n", qPrintable(cls));
    ...
    if (!g_shadowDir.empty() && !g_shadows.empty()) {
        for (auto &u : g_shadows) {
            ...
            qtdEmitNotice(sf, inPath, "a shadow document for one refused expression");
