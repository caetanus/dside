...
"
    ...
    void qmlSuite(QtdBinding b, string tag) {
        foreach (dc; DCS) {
            ...
            // ...and a D method that RETURNS, called from QML. Qt draws the slot/invokable line at
    ...
    foreach (qmlFile; corpus) {
        ...
        foreach (dc; DCS) {
            ...
            static immutable string[] labelsGap = [
                ...
                "QDelegateOuterName", "QDelegateParentIsNotOuter", "QRequiredVarRole",
            ];
            ...
            version (Windows) {
                ...
                string[] ls = [
                    // `--labels` is not optional: without a mode flag the tool DUMPS the
                    // generated D source, so the props file held a D header and the oracle
                    // reported `no object at path '// GENERATED by qmltc-d from …'`. Dropped
                    // when this site was first converted to PowerShell.
