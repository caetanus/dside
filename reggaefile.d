...
Target[] uicheckTargets(string root, QtdBinding ex) {
    ...
    foreach (dc; DCS) {
        ...
        ts ~= Target.phony("uicheck-" ~ dc, runOffscreen("$in"), [bin]);
    }
    return ts;
}
...
Target[] corpusCheckTargets(string root, QtdBinding ex) {
    ...
    foreach (dc; DCS) {
        ...
        ts ~= Target.phony("corpus-check-" ~ dc, runOffscreen("$in"), [bin]);
    }
    return ts;
}
...
Target[] qmlTrTargets(string root, QtdBinding qml, string tag = "") {
    ...
    foreach (dc; DCS) {
        ...
        if (lrelease.length) {
            ...
            ts ~= Target.phony("tr" ~ tag ~ "-" ~ dc, runOffscreen("$in"), [bin, qmT]);
        } else {
            ts ~= Target.phony("tr" ~ tag ~ "-" ~ dc, runOffscreen("$in"), [bin]);
        }
    }
    return ts;
}
...
Target[] qmlAotTargets(string root, QtdBinding qml) {
    ...
    foreach (dc; DCS) {
        ...
        ts ~= Target.phony("qmlaot-" ~ dc, runOffscreen("$in"), [bin]);
    }
    return ts;
}
