...
// A QAbstractListModel WRITTEN IN D, on the QML binding — Qt5 AND Qt6. listmodel_test.d proves it
// against a real ListView on Qt6's Quick binding; this runs the same model and the same checks
// against an Instantiator, which needs no QtQuick and so exists on both Qt versions.
//
...
import qt.qml.qabstractlistmodel, qt.qml.qmodelindex, qt.qml.qvariant;
import qt.qml.qtvirt;                   // __QAbstractListModel_vnames/_prot for the mixin
import cppq = qt.qml.qobject;           // the C++ QObject (renamed: `@QObject` below is qtmoc's)
...
mixin(qrcRegister(import("listmodel_qml.qrc"), "qt.qml"));
...
@QObject class Driver {
    Signal!() step;
    Signal!() rowsChanged;
    // Typed as the C++ QObject: `QObject*` is a meta-type every Qt knows, and a view's `model`
...
void main() {
    ...
    engine.load("qrc:/listmodel_qml.qml");
    ...
    writeln("qmllistmodel OK: a D QAbstractListModel drives an Instantiator incrementally ",
