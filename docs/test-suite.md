...
| **uic (CTFE)** | `uic`, `dialog`, `tabs`, `mainwin`, `hello`, `egroup`, `combo`, `spacer`, `icon`, `uicheck`, `corpus-check` | `mixin(uiForm(import(".ui")))` → typed struct. `uicheck` + `corpus-check` are **differential**: our tree must serialize identically to `QUiLoader`'s over the whole Qt baseline corpus (**60/60**, 2 lines waived). The dump compares the widget tree AND the layout structure — item order, grid cell/span, per-item alignment and stretch, spacer size policy, and margins — because comparing only named QObjects and their properties left unnamed layouts, spacers and item alignment invisible on both sides (that blind spot is what let `gridalignment.ui` pass with its alignment dropped). The waivers are per-line, justified in the test against the output of Qt's own `uic`, and asserted to still apply. |
