...
and what it measured at the time, in the order it happened. It is kept because the *how it was
found* is the part that does not survive summarising — the wrong-document bug, the interceptor, the
value axis and the flaky filter were all found by a method, and the method is reusable.

For what qmltc-d IS and how to use it, read **`qmltc-d.md`**. For how it compares with Qt's own
`qmltc`, read **`qmltc-d-vs-qmltc.md`**. Both of those carry the CURRENT numbers; the ones below are
whatever was true on the day they were written, and several have been superseded. Where a figure
here disagrees with the reference, the reference is right.
